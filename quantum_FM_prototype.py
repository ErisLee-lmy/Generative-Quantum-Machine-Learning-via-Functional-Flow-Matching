...
class QuantumVelocityField(nn.Module):
    ...
    def forward(self, t, bloch):
        params = self.param_net(t, bloch)[0]
...
for epoch in range(epochs):
    ...
    for i in range(batch_size):
        params = velocity_field.param_net(t[i].unsqueeze(0), xt[i].unsqueeze(0))
        bloch_pred = quantum_circuit(params[0])
        v_pred_list.append(bloch_pred.unsqueeze(0))  # 保证 [1,3]
...
with torch.no_grad():
    for _ in range(n_traj):
        ...
        traj_points = []
        for ti in timesteps:
            t_tensor = torch.tensor([[ti]], dtype=dtype)
            params = velocity_field.param_net(t_tensor, xt)
            v_pred = quantum_circuit(params[0]).unsqueeze(0)
            dt = 1.0 / len(timesteps)
            xt = xt + v_pred * dt
            xt = xt / torch.norm(xt)
            traj_points.append(xt.squeeze(0).detach().cpu().numpy())
